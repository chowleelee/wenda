          ...
          <ul class="listen-content" ref="tes">
            <li class="a-feed" v-for="(item,index) in ask_list">
              <div class="header">
                ...
                <router-link :to="'/problemDetails/'+item.id"><span class="header-title">{{item.real_name}}</span></router-link>
              </div>
              ...
              <div class="footer">
                ...
                <div class="footer-answer" v-if="item.answer_flag == 2">
                  <a @click="payToListen(item.id,item.user_id,item.vip_id)" class="bubble bubble-green" v-show="!item.listened">
                    <span class="bubble-tail"></span>
                    <span class="wave3"></span>
                    <span class="bubble-tip">1元偷偷看</span>
                    <!-- <span class="pay-look">1元偷偷看</span> -->
                  </a>
                  <my-text v-show="item.listened" :flag="item.answer_flag" :content="item.answer_content"></my-text>
                </div>
                <span class="tips-right" :class="{'tips-text':item.answer_flag == 2&&item.listened}">浏览 {{item.listen_num}}</span>
              </div>
            </li>
          </ul>
